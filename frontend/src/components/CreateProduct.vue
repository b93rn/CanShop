  ...
  <SlidePage v-if="creatingProduct">
    <div class="createProduct">
      ...
      <div class="quantitySelectors">
        <NumberSelector title="Bedrag" @amountChanged="setPrice" />
        <NumberSelector title="Inkoop" @amountChanged="setPurchasePrice" />
        <NumberSelector title="Aantal" @amountChanged="setAmount" :numbers="[24, 12, 10, 5, 2, 1]" :isCurrency="false"/>
      </div>
      <div class="buttons">
        <Button
          title="Annuleren"
...
export default {
  ...
  methods: {
    cancelCreatingProduct: function() {
      ...
      this.product.purchasePrice = 0.00
      this.product.amount = 0
      this.$store.commit('TOGGLE_CREATING_PRODUCT')
    },
    submitProduct: function() {
      if(this.product.name && this.product.price && this.product.purchasePrice){
